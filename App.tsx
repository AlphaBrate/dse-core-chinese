...
const App: React.FC = () => {
  ...
  const [translationType, setTranslationType] = useState<'word' | 'sentence'>('sentence');
  const [quizSessions, setQuizSessions] = useState<Record<string, { index: number, answer: string }>>({});
    ...
    if (savedProgress) {
      try {
        ...
        Object.keys(parsed).forEach(qid => {
          migrated[qid] = Array.isArray(parsed[qid]) ? parsed[qid] : [parsed[qid]];
        });
  ...
  const toggleMistake = (qid: string, add: boolean) => {
    ...
    if (add) {
      setMistakeHistoryIds(prev => {
        ...
        const next = [...prev, qid];
        localStorage.setItem('dse_mistake_history', JSON.stringify(next));
        return next;
      });
    }
  };
  ...
  const saveProgress = (qid: string, answer: string, result: any) => {
    setProgress(prev => {
      ...
      const next = {
        ...prev,
        [qid]: [...currentHistory, { answer, result, timestamp: Date.now() }]
      };
  ...
  const updateQuizSession = (topic: string, index: number, answer: string) => {
        ...
        <div className="max-w-4xl mx-auto py-8 px-4 min-h-[60vh]">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 animate-pulse">
              ...
              <p className="text-coreMuted font-medium">正在載入專屬數據庫...</p>
            </div>
          ) : (
            <>
              ...
              {mode === AppMode.Quiz && (
                <QuizMode 
                  topic={selectedTopic} 
                  questions={questions}
                  onMistake={(qid, isMistake) => toggleMistake(qid, isMistake)}
                  mistakeIds={mistakeIds}
                  progress={progress}
                  onSaveProgress={saveProgress}
                  sessionState={quizSessions[selectedTopic]}
                  onUpdateSession={(idx, ans) => updateQuizSession(selectedTopic, idx, ans)}
                />
              )}
