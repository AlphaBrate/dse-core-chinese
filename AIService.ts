...
// Vite intercepts this route and proxies it directly to NVIDIA securely
const NIM_URL = "/api/nvidia/chat/completions";
const MODEL_NAME = localStorage.modelID || "nvidia/nemotron-3-ultra-550b-a55b";

const errorNotes = `Failed to connect with ${MODEL_NAME}. Try checking your local development server or validating your custom API key inside localStorage.user_api_key.`;

console.log("Using Model " + MODEL_NAME);

/**
 * Core Fetch Wrapper for NIM via Vite Proxy
 */
async function callNIM(
    ...
    const response = await fetch(NIM_URL, {
        method: "POST",
        headers: {
            // Extracts the user's custom API key from localStorage dynamically
            Authorization: `Bearer ${localStorage.getItem("user_api_key") || ""}`,
            "HTTP-Referer": "https://aphbrt.web.app/dse/chinese", 
            "X-Title": "AlphaBrate DSE", 
    ...
    if (!response.ok) {
        ...
        throw new Error(
            errorData.error?.message || "NIM Request via Vite Proxy Failed"
        );
    }
...
export const gradeAnswer = async (
    ...
    重要提示：
    - 你必須只輸出一個 JSON 對象，不得輸出任何其他東西，第一個字符必須是 「{」必須以「}」結尾。
...
export const getQuestionHint = async (question: Question): Promise<string> => {
  ...
  **重要限制：** - 直接輸出提示內容，嚴禁包含「提示：」或「Hint:」等任何標籤前綴。
  ...
  `;
    ...
    try {
        const content = await callNIM(systemInstruction, promptText);
        if (localStorage.printAIOutput === "true") console.log(content);
